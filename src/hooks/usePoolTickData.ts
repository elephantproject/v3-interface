...
import { FeeAmount, nearestUsableTick, Pool, TICK_SPACINGS, tickToPrice } from 'elephantswapv3-sdk'
...
import {
  ...
  TickData,
  Ticks,
} from 'graphql/thegraph/AllV3TicksQuery'
...
function useTicksFromTickLens(
  ...
  useEffect(() => {
    if (!IsSyncing && !isLoading && !isError && isValid) {
      setTickDataLatestSynced(tickData.sort((a, b) => Number(a.tick) - Number(b.tick)))
    }
...
function useTicksFromSubgraph(
  ...
  const query = useQuery<AllV3TicksQuery, AllV3TicksQueryVariables>(ALL_V3_TICKS_QUERY, {
    variables: { poolAddress: normalizedPoolAddress ?? '', skip },
    skip: !poolAddress,
    pollInterval: ms`30s`,
    client: apolloClient,
  })

  return { ...query, poolAddress: normalizedPoolAddress }
}
...
function useAllV3Ticks(
  ...
  useEffect(() => {
    if (data?.ticks.length) {
      setSubgraphTickData((tickData) => {
        const ticksByIndex = new Map(tickData.map((tick) => [tick.tick, tick]))
        data.ticks.forEach((tick) => ticksByIndex.set(tick.tick, tick))
        return [...ticksByIndex.values()].sort((a, b) => Number(a.tick) - Number(b.tick))
      })
