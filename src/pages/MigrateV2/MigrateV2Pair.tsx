...
import { BlueCard, DarkGrayCard, LightCard, OutlineCard, YellowCard } from 'components/Card'
...
import { FeeAmount, Pool, Position, priceToClosestTick, TickMath } from 'elephantswapv3-sdk'
import { CurrencyAmount, Fraction, Percent, Price, Token } from 'elephantswapv3-sdk-core'
...
type MigrationPriceMode = 'v2' | 'custom'

const PriceModeControl = styled.div`
...
const PriceModeButton = styled.button<{ active: boolean }>`
...
const CustomPriceInput = styled(NumericalInput)`
  font-size: 24px;
  text-align: left;
  width: 100%;
`
...
function V2PairMigration({
  ...
  const [migrationPriceMode, setMigrationPriceMode] = useState<MigrationPriceMode>('v2')
  const [customMigrationPrice, setCustomMigrationPrice] = useState('')
  ...
  const displayedV2SpotPrice = invertPrice ? v2SpotPrice.invert() : v2SpotPrice
  const displayedBaseToken = invertPrice ? token1 : token0
  const displayedQuoteToken = invertPrice ? token0 : token1
  const parsedCustomMigrationPrice = useMemo(
    () => tryParsePrice(displayedBaseToken, displayedQuoteToken, customMigrationPrice),
    [customMigrationPrice, displayedBaseToken, displayedQuoteToken]
  )
  const normalizedCustomMigrationPrice = invertPrice ? parsedCustomMigrationPrice?.invert() : parsedCustomMigrationPrice
  const selectedMigrationPrice = migrationPriceMode === 'v2' ? v2SpotPrice : normalizedCustomMigrationPrice
  const selectedMigrationTick = useMemo(() => {
    if (!selectedMigrationPrice) return undefined
    try {
      return priceToClosestTick(selectedMigrationPrice)
    } catch {
      return undefined
    }
  }, [selectedMigrationPrice])
  const invalidMigrationPrice = noLiquidity && migrationPriceMode === 'custom' && selectedMigrationTick === undefined
  const rangeReferencePrice = pool?.token0Price ?? selectedMigrationPrice

  const handleMigrationPriceMode = useCallback(
    (mode: MigrationPriceMode) => {
      setMigrationPriceMode(mode)
      if (mode === 'custom' && !customMigrationPrice) {
        setCustomMigrationPrice(displayedV2SpotPrice.toSignificant(8))
      }
    },
    [customMigrationPrice, displayedV2SpotPrice]
  )
  ...
  const handleSetCustomRange = useCallback(() => {
    ...
    const displayedSpotPrice = invertPrice ? rangeReferencePrice?.invert() : rangeReferencePrice
    if (!displayedSpotPrice) return
    const currentPrice = Number(displayedSpotPrice.toSignificant(12))
    if (!Number.isFinite(currentPrice) || currentPrice <= 0) return

    onLeftRangeInput((currentPrice * 0.8).toPrecision(8))
    onRightRangeInput((currentPrice * 1.2).toPrecision(8))
  }, [fullRange, invertPrice, onLeftRangeInput, onRightRangeInput, rangeReferencePrice])

  // the v3 tick is either the pool's tickCurrent, or the tick closest to the v2 spot price
  const tick = pool?.tickCurrent ?? selectedMigrationTick
  // the price is either the current v3 price, or the price at the tick
  const sqrtPrice = pool?.sqrtRatioX96 ?? (typeof tick === 'number' ? TickMath.getSqrtRatioAtTick(tick) : undefined)
  const position =
    typeof tick === 'number' &&
    sqrtPrice &&
  ...
  const approve = useCallback(() => approveManually(), [approveManually])
      ...
      <LightCard>
        <AutoColumn gap="lg">
          ...
          {noLiquidity && (
            <AutoColumn gap="md">
              <ThemedText.DeprecatedLabel>
                <Trans>V3 pool starting price</Trans>
              </ThemedText.DeprecatedLabel>
              <PriceModeControl>
                <PriceModeButton
                  active={migrationPriceMode === 'v2'}
                  aria-pressed={migrationPriceMode === 'v2'}
                  onClick={() => handleMigrationPriceMode('v2')}
                  type="button"
                >
                  <Trans>Use V2 price</Trans>
                </PriceModeButton>
                <PriceModeButton
                  active={migrationPriceMode === 'custom'}
                  aria-pressed={migrationPriceMode === 'custom'}
                  onClick={() => handleMigrationPriceMode('custom')}
                  type="button"
                >
                  <Trans>Custom price</Trans>
                </PriceModeButton>
              </PriceModeControl>

              {migrationPriceMode === 'custom' && (
                <OutlineCard padding="12px">
                  <CustomPriceInput
                    aria-label="Custom V3 pool starting price"
                    value={customMigrationPrice}
                    onUserInput={setCustomMigrationPrice}
                  />
                  <ThemedText.DeprecatedBody color="textSecondary" fontSize={12} marginTop="8px">
                    <Trans>
                      {displayedQuoteToken.symbol} per {displayedBaseToken.symbol}
                    </Trans>
                  </ThemedText.DeprecatedBody>
                  {invalidMigrationPrice && customMigrationPrice && (
                    <ThemedText.DeprecatedError error fontSize={12} marginTop="8px">
                      <Trans>Enter a valid starting price.</Trans>
              ...
              <BlueCard style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <ThemedText.DeprecatedBody
                  ...
                  textAlign="center"
                >
                  {migrationPriceMode === 'v2' ? (
                    <Trans>
                      This new V3 pool will be initialized at the current {isNotUniswap ? 'SushiSwap' : 'V2'} price of{' '}
                      {displayedV2SpotPrice.toSignificant(6)} {displayedQuoteToken.symbol} per{' '}
                      {displayedBaseToken.symbol}.
                    </Trans>
                  ) : (
                    <Trans>The new V3 pool will be initialized at your custom price.</Trans>
                  )}
                </ThemedText.DeprecatedBody>
                <ThemedText.DeprecatedBody fontWeight={500} textAlign="center" fontSize={14}>
                  <Trans>Your transaction cost will be higher because it includes creating the pool.</Trans>
                </ThemedText.DeprecatedBody>
              </BlueCard>
            </AutoColumn>
          )}
          ...
          <PresetsButtons
            ...
            onSetFullRange={getSetFullRange}
          />
          ...
          <RowBetween justify="flex-end">
            <RateToggle
              ...
              handleRateToggle={() => {
                onLeftRangeInput('')
                onRightRangeInput('')
                if (migrationPriceMode === 'custom' && parsedCustomMigrationPrice) {
                  setCustomMigrationPrice(parsedCustomMigrationPrice.invert().toSignificant(8))
                }
