...
import { Currency, CurrencyAmount, TradeType } from 'elephantswapv3-sdk-core'
...
export function parseLocalActivity(
  ...
  try {
    ...
    const customTitle =
      details.info.type === TransactionType.CUSTOM
        ? status === TransactionStatus.Pending
          ? details.info.pendingTitle
          : status === TransactionStatus.Confirmed
          ? details.info.confirmedTitle
          : details.info.failedTitle
        : undefined
    const defaultFields = {
      hash: details.hash,
      chainId,
      title: customTitle ?? getActivityTitle(details.info.type, status),
      status,
    ...
    if (info.type === TransactionType.CUSTOM) {
      additionalFields = { descriptor: info.descriptor }
    } else if (info.type === TransactionType.SWAP) {
