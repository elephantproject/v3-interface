...
import { PERMIT2_ADDRESS } from 'elephantswap-universal-router-sdk'
import { Token, TradeType as MockTradeType } from 'elephantswapv3-sdk-core'
...
describe('parseLocalActivity', () => {
  it('labels Harmony native swap amounts as ONE', () => {
    const chainId = SupportedChainId.HARMONY
    const details = {
      info: {
        type: MockTxType.SWAP,
        tradeType: MockTradeType.EXACT_INPUT,
        inputCurrencyId: 'ETH',
        inputCurrencyAmountRaw: mockCurrencyAmountRaw,
      ...
      ...mockCommonFields('0xharmony_one', mockAccount2, MockTxStatus.Confirmed),
    } as unknown as TransactionDetails

    expect(parseLocalActivity(details, chainId, {})).toMatchObject({
      descriptor: '1.00 ONE for 1.00 ONE',
    })
  })
