...
import { CurrencyAmount, Percent, Token } from 'elephantswapv3-sdk-core'
...
interface PositionCardProps {
  ...
  onOneLockedChange?: (address: string, oneLocked: number | undefined) => void
}
...
export default function FullPositionCard({ pair, border, stakedBalance, onOneLockedChange }: PositionCardProps) {
  ...
  const oneLocked = useMemo(() => {
    const wrappedOne = chainId ? WRAPPED_NATIVE_CURRENCY[chainId] : undefined
    if (!wrappedOne || !token0Deposited || !token1Deposited) return undefined

    const wrappedOneAmount = pair.token0.equals(wrappedOne)
      ? token0Deposited
      : pair.token1.equals(wrappedOne)
      ? token1Deposited
      : undefined
    if (!wrappedOneAmount) return 0

    const amount = Number(wrappedOneAmount.toSignificant(12))
    return Number.isFinite(amount) ? amount : undefined
  }, [chainId, pair.token0, pair.token1, token0Deposited, token1Deposited])

  useEffect(() => {
    onOneLockedChange?.(pair.liquidityToken.address, oneLocked)
  }, [onOneLockedChange, oneLocked, pair.liquidityToken.address])
