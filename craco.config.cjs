...
function getCommitHash() {
  if (process.env.REACT_APP_GIT_COMMIT_HASH) return process.env.REACT_APP_GIT_COMMIT_HASH
...
// Keep the development server focused on compiling and serving the app. Whole-project linting and type checking
// block the first render for a long time in this repository; run `yarn lint` and `yarn typecheck` explicitly, or
// opt back into the webpack checks with ENABLE_DEV_CHECKS=true.
const shouldLintOrTypeCheck = !isProduction && process.env.ENABLE_DEV_CHECKS === 'true'
  ...
  webpack: {
    ...
    configure: (webpackConfig) => {
      ...
      webpackConfig.plugins = webpackConfig.plugins
        ...
        .filter((plugin) => {
          // Keep the default development server non-blocking. These checks remain available through the dedicated
          // lint/typecheck scripts and can be restored in webpack with ENABLE_DEV_CHECKS=true.
          if (
            !shouldLintOrTypeCheck &&
